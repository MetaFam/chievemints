import { ak as CID, al as ipfsLinkPattern, u as useWeb3, N as NETWORKS, r as reactExports, R as React, e as jsx, L as Link } from "./index-f164924e.js";
var Space_Separator = /[\u1680\u2000-\u200A\u202F\u205F\u3000]/;
...
const extractMessage = (error) => error.reason ?? error?.details ?? error?.error?.message ?? error?.data?.message ?? error?.message ?? (typeof error === "string" ? error : `𝑼𝒏𝒌𝒏𝒐𝒘𝒏 𝑬𝒓𝒓𝒐𝒓: ${lib.stringify(error, null, 2)}`);
...
const UncontrolledTabs = (props) => {
  ...
  const { children, className, disabledTabClassName, domRef, focus, forceRenderTabPanel, onSelect, selectedIndex, selectedTabClassName, selectedTabPanelClassName, environment, disableUpDownKeys, disableLeftRightKeys, ...attributes2 } = { ...defaultProps$4, ...props };
...
const Tabs = (props) => {
  const { children, defaultFocus, defaultIndex, focusTabOnClick, onSelect, ...attributes2 } = { ...defaultProps$3, ...props };
  const [focus, setFocus] = reactExports.useState(defaultFocus);
  const [mode] = reactExports.useState(getModeFromProps(attributes2));
  ...
  let subProps = { ...props, ...attributes2 };
...
const TabList = (props) => {
  const { children, className, ...attributes2 } = { ...defaultProps$2, ...props };
...
const Tab = (props) => {
  ...
  const { children, className, disabled, disabledClassName, focus, id, selected, selectedClassName, tabIndex, tabRef, ...attributes2 } = { ...defaultProps$1, ...props };
  reactExports.useEffect(() => {
    if (selected && focus) {
...
const TabPanel = (props) => {
  const { children, className, forceRender, id, selected, selectedClassName, ...attributes2 } = { ...defaultProps, ...props };
...
//# sourceMappingURL=TokenFilterForm.module-7fdc84b3.js.map
